...
namespace WhiteAds.API.Controllers
{
    ...
    /// </summary>
    public class MonedaController : ApiBaseController
    {
        /// <summary>
        /// Método Para Mostrar por Lista de Moneda
        /// </summary>
        ...
        public HttpResponseMessage Get(int? page = 1, int? start = 0, int? limit = 20, string filter = "", string sort = "")
        {
            ...
            AppPagedResponse<List<MonedaModel>> response = new AppPagedResponse<List<MonedaModel>>() { Data = null };

            var db = monedaSvc.Load();
        ...
        {
            AppResponse<MonedaModel> response = new AppResponse<MonedaModel>() { Data = null };
            var db = monedaSvc.Load(id);
        ...
        /// <returns></returns>
        // POST: api/Moneda
        public HttpResponseMessage Post([FromBody] MonedaModel value)
        {
            AppResponse<Moneda> response = new AppResponse<Moneda>() { Data = null };
            var db = monedaSvc.Save(MapToDB(value));
            response.Data = db;
            return Request.CreateResponse(HttpStatusCode.OK, response);
        }
        ...
        /// <param name="value"></param>
        // PUT: api/Moneda/5
        public HttpResponseMessage Put(int id, [FromBody] MonedaModel value)
        {
            AppResponse<Moneda> response = new AppResponse<Moneda>() { Data = null };
            var db = monedaSvc.Save(MapToDB(value));
            response.Data = db;
